...
class MultiPickerDialog: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    func setupView() {
        ...
        self.picker.register(DialogCell.self, forCellReuseIdentifier: "cell")
...
//        sceneDelegate.window?.endEditing(true)
    ...
    @objc func buttonTapped(sender: UIButton!) {
        if sender.tag == kPickerDialogDoneButtonTag {
           ...
           // if let indexPathsForSelectedRows = self.picker.indexPathsForSelectedRows{
            for indexPath  in self.selectedIndex{
                let cell = self.pickerData[indexPath.row]
                theSelectedValues.append(["value":cell["value"] ?? "0", "display":cell["display"] ?? ""])
            }
            //}
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell: UITableViewCell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? UITableViewCell {
            cell.textLabel?.text = self.pickerData[indexPath.row]["display"]
            cell.contnetIdentifier = self.pickerData[indexPath.row]["value"]
        ...
        /*
        if let cell =  tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? DialogCell{
            //        if ((cell == nil)) {
    ...
    internal func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        print("IndexPaths ==== \(selectedIndex)")
    }
    
    internal func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        if let indexToDelete = self.selectedIndex.firstIndex(of: indexPath){
            self.selectedIndex.remove(at: indexToDelete)
        }
