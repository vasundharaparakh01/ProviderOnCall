...
open class EPSignatureViewController: UIViewController {
    ...
    @IBOutlet weak var btnLoadSignature: UIButton!
    ...
    override open func viewDidLoad() {
        ...
        self.setLoadSignatureButton()
        
    }
    func setLoadSignatureButton(){
        let docPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first
        let filePath = (docPath! as NSString).appendingPathComponent("sig.data")
        if let path = self.signatureView.getPath(filePath){
            self.btnLoadSignature.isHidden = false
        }else{
            self.btnLoadSignature.isHidden = true
        }
    ...
    @IBAction func btnSignatureSave_Action(_ sender: Any) {
        if let signature = signatureView.getSignatureAsImage() {
            if switchSaveSignature.isOn {
                let docPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first
                let filePath = (docPath! as NSString).appendingPathComponent("sig.data")
    ...
    @IBAction func btnSignatureCancel_Action(_ sender: Any) {
        signatureDelegate?.epSignature!(self, didCancel: NSError(domain: "EPSignatureDomain", code: 1, userInfo: [NSLocalizedDescriptionKey:"User not signed"]))
        dismiss(animated: true, completion: nil)
    }
    @IBAction func btnSignatureClear_Action(_ sender: Any) {
        signatureView.clear()
    }
    @IBAction func btnSignatureLoad_Action(_ sender: Any) {
...
//        }
        ...
        if Device.IS_IPAD{
            addActionSheetForiPad(actionSheet: action)
        }
